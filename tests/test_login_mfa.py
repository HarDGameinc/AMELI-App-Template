...
# ---- Verify with email when SMTP is broken (regression: must not 500) ----
...
def test_choose_email_method_does_not_500_on_smtp_failure(
    ...
    """Regression: POST choose_method=email used to raise
    SMTPAuthenticationError (or any non-ValueError) and the view's
    except-only-ValueError turned it into a 500. The flow must now
    bounce back to /login/verify-mfa/ with a flash message."""
    ...
    with patch.object(EmailMessage, "send", side_effect=ConnectionRefusedError("smtp down")):
        response = client.post("/login/verify-mfa/", {"choose_method": "email"})

    assert response.status_code in {302, 303}
    assert response["Location"].endswith("/login/verify-mfa/")
    # Audit row for the failure is written so an operator can investigate.
    from ameli_web.audit.models import AuditEvent
    audit = AuditEvent.objects.filter(action="mfa_email_login_send_failed").last()
    assert audit is not None
    assert audit.payload.get("error_class") == "ConnectionRefusedError"
...
def test_get_verify_mfa_with_email_does_not_500_on_smtp_failure(
    ...
    """Same regression on the GET path — first render also tries to
    auto-send a code; SMTP failure must not 500 the page."""
    from unittest.mock import patch
    from django.core.mail import EmailMessage

    settings.AUDIT_HMAC_KEY = "k"
    client.post("/login/", {"username": "admin", "password": ADMIN_PASSWORD})
    # Pre-select email so the GET goes straight to the send-on-render branch.
