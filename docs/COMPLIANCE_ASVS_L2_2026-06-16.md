...
| 23 | Enable branch protection on `main` (require PR + CI green) | S | **closed-2026-06-18** (template-side) | Runbook in `docs/OPERATIONS.md` § "Branch protection on main" pins both UI clicks and `gh api` payload. **Use classic Branch protection rules, NOT Rulesets** — the newer Rulesets feature is silently no-op on private repos under the Free plan ("Active" but unenforced) until the org upgrades to GitHub Team; the classic rules are enforced on every plan. Operator applied 2026-06-18 after observing the ruleset warning. |
