...
| 23 | Enable branch protection on `main` (require PR + CI green) | S | **closed-2026-06-18** (template-side) | Runbook in `docs/OPERATIONS.md` § "Branch protection on main": both UI clicks and `gh api` payload pinned. Repo admin applies once via Settings → Branches. Cannot be set from MCP (admin scope not exposed). |
