...
| R-03 | Swagger/ReDoc CDN bundles load unsigned in dev | **Closed 2026-06-16** | `/docs` and `/redoc` now refuse to render outside `dev` when any SRI is empty (503 + operator-actionable body). Helper `tools/sri_compute.py` generates the hashes from a host with public internet. Operator can opt out explicitly via `AMELI_APP_OPENAPI_SRI_REQUIRED=false` (informed risk acceptance, e.g. air-gapped CDN mirror). |
| R-04 | Dependency pins use `>=` rather than `==` with hashes | **Closed 2026-06-18** | Roadmap item #14 shipped `requirements.lock` + `requirements-dev.lock` generated by `pip-compile --generate-hashes`. CI and the deploy script install with `pip install --require-hashes -r requirements.lock -r requirements-dev.lock`; any wheel whose archive hash does not match is refused. `tests/test_lockfile_hashes.py` pins the contract: lockfiles exist, every top-level dep is locked, every locked entry carries at least one `--hash=`, CI + deploy reference the lock, `pip-tools` is in dev deps. |
| R-05 | No automatic AV scan on avatar upload | **Closed 2026-06-17** | `AMELI_APP_AV_ENDPOINT` plumbs clamd over TCP (`tcp://host:port`) or HTTP (`http(s)://...`) into the avatar upload path. INFECTED uploads are rejected + audited as `avatar_upload_av_rejected`. Endpoint outages fail-open + audit as `avatar_upload_av_check_failed`. Format whitelist + pixel cap + byte cap remain in place as defence-in-depth. See `docs/OPERATIONS.md` "Avatar AV scan". |
...
| R-09 | Sudo grant has no replay nonce within its window — any authenticated request within the TTL can act | Accepted by design | Sudo TTL is short (default 300 s, env `AMELI_APP_SUDO_TTL_SECONDS`); the trade-off is documented in `docs/COMPLIANCE_ASVS_L2_2026-06-16.md` as ASVS V11.1.5 (acceptable). Stronger replay prevention would require per-request nonces on every state-changing admin endpoint — overkill given the short window and the per-action audit trail. |
| R-11 | `AMELI_APP_AUDIT_HMAC_KEY` could be empty in prod, silently disabling chain integrity (ASVS V7.3.2, V6.3.1) | **Closed 2026-06-19** | Boot guard added in `src/ameli_web/settings.py` mirroring the `MFA_ENCRYPTION_KEY` pattern: outside `dev`, an empty `AMELI_APP_AUDIT_HMAC_KEY` raises `RuntimeError` at module load. Caught by the 2026-06-19 independent security re-audit. `tests/test_settings_boot_guards.py` pins the guard. |
| R-12 | `AMELI_APP_AV_ENDPOINT` scheme not validated at boot — `file://` or bare host fell back to fail-open at upload time | **Closed 2026-06-19** | Boot guard added: only `tcp://`, `http://`, `https://` accepted; anything else raises `RuntimeError`. Forces the operator to fix the env BEFORE the first upload instead of discovering a silent fail-open at runtime. |
| R-10 | No explicit `DATA_UPLOAD_MAX_MEMORY_SIZE` override — Django default 2.5 MB | Accepted | Documented as ASVS V13.1.5; for the 3 MB avatar use case this kicks form parsing to disk, which is benign. An operator handling significantly larger uploads should raise the limit; the current default protects against memory exhaustion at the cost of disk I/O on the boundary. |
